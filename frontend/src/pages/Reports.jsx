...
export default function Reports() {
  ...
  const exportReportToPDF = async () => {
    ...
    try {
      ...
      doc.text('MindAlchemy Progress Report', 105, yPos, { align: 'center' })
      yPos += 10
      
      // Date Range
      doc.setFontSize(12)
      doc.setTextColor(0, 0, 0)
      const dateText = `${format(dateRange.start, 'MMM d, yyyy')} - ${format(dateRange.end, 'MMM d, yyyy')}`
      doc.text(dateText, 105, yPos, { align: 'center' })
      yPos += 15
      
      // Mood Statistics
      if (reportData.moodStats) {
        doc.setFontSize(16)
        doc.setTextColor(99, 102, 241)
        doc.text('Mood Statistics', 20, yPos)
        yPos += 8
        
        doc.setFontSize(11)
        doc.setTextColor(0, 0, 0)
        doc.text(`Total Mood Logs: ${reportData.moodStats.total}`, 20, yPos)
        yPos += 7
        
        if (reportData.moodStats.distribution) {
          doc.text('Mood Distribution:', 20, yPos)
          yPos += 6
          Object.entries(reportData.moodStats.distribution).forEach(([mood, count]) => {
            doc.text(`  • ${mood.charAt(0).toUpperCase() + mood.slice(1)}: ${count}`, 25, yPos)
            yPos += 6
            if (yPos > 270) {
              doc.addPage()
              yPos = 20
      ...
      if (reportData.journalStats) {
        ...
        doc.setFontSize(11)
        doc.setTextColor(0, 0, 0)
        doc.text(`Total Journal Entries: ${reportData.journalStats.total}`, 20, yPos)
        yPos += 7
        
        if (Object.keys(reportData.journalStats.byType || {}).length > 0) {
          doc.text('Entries by Type:', 20, yPos)
          yPos += 6
          Object.entries(reportData.journalStats.byType).forEach(([type, count]) => {
      ...
      if (reportData.sessionStats) {
        doc.setFontSize(16)
        doc.setTextColor(99, 102, 241)
        doc.text('Chat Sessions', 20, yPos)
        yPos += 8
        
        doc.setFontSize(11)
        doc.setTextColor(0, 0, 0)
        doc.text(`Total Sessions: ${reportData.sessionStats.total}`, 20, yPos)
        yPos += 6
        doc.text(`Total Messages: ${reportData.sessionStats.totalMessages}`, 20, yPos)
        yPos += 6
        doc.text(`Average Sentiment: ${reportData.sessionStats.averageSentiment || 'neutral'}`, 20, yPos)
        yPos += 10
      }
      
      // AI Insights (if available)
      if (insights && insights.overall_assessment) {
        if (yPos > 250) {
          doc.addPage()
          yPos = 20
        }
        ...
        doc.setFontSize(11)
        doc.setTextColor(0, 0, 0)
        const assessmentLines = doc.splitTextToSize(insights.overall_assessment, 170)
      ...
      for (let i = 1; i <= pageCount; i++) {
        ...
        doc.text('Generated by MindAlchemy', 105, 290, { align: 'center' })
      }
      
      // Save PDF
      const filename = `mindalchemy-report-${reportType}-${format(new Date(), 'yyyy-MM-dd')}.pdf`
      doc.save(filename)
    } catch (error) {
